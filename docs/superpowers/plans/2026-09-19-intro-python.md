...
- [ ] **Step 1: Проверка, которая пока падает**

`node инструменты/checksteps.js m0l2` → «m0l2: нет урока с шагами» (урока нет).

- [ ] **Step 2: `Engine.python` — одна загрузка на всех**
...
- [ ] **Step 3: `Steps` — запуск и проверка Python**
...
- [ ] **Step 4: Страница пошагового урока**
...
- [ ] **Step 6: Урок 0.2 — карта и шаги 1–2**
...
- [ ] **Step 7: `checksteps.js` — Python-шаги**
...
- [ ] **Step 1:** написать шаги 3–9 по таблице спецификации. Эталоны
...
- [ ] **Step 1: Проверка, которая пока падает.** Браузер, `#m0l1`:
  ссылка «дальше» в `#lnav` ведёт на `#m0l2` (должна на `#m1l1`).
- [ ] **Step 2: `Course`.** После сборки `flat`:
...
- [ ] **Step 5: Проверка.** `#m0l1` → «дальше» `#m1l1`; `#m1l8` →
...
- [ ] **Step 6:** коммит `0.2 проходят перед модулем 2: порядок прохождения по полю before`.
